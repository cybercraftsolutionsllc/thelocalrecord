...
export function SiteShell({ children }: SiteShellProps) {
  ...
  return (
    <div className="min-h-screen text-ink">
      <header className="border-b border-ink/10 bg-white/76 backdrop-blur">
          ...
          <nav className="flex flex-wrap gap-2 text-sm lg:justify-end">
            {visibleNavItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-full border border-ink/10 bg-white/82 px-4 py-2 text-ink/80 transition hover:border-moss/30 hover:bg-sky/40 hover:text-moss"
              >
                {item.label}
              </Link>
            ))}
      ...
      <footer className="border-t border-ink/10 bg-white/82 backdrop-blur">
