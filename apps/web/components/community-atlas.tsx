...
export function CommunityAtlas({ slug }: CommunityAtlasProps) {
  return (
    <section className="rounded-lg border border-ink/[0.10] bg-white p-5">
      ...
      <p className="mt-5 rounded-lg border border-ink/[0.10] bg-sand/[0.35] px-4 py-3 text-xs leading-6 text-ink/[0.62]">
        This is not an official parcel search. It is a resident-friendly scan of
        source-linked local records.
      </p>

      <div className="mt-5 divide-y divide-ink/[0.08] border-t border-ink/[0.08] pt-2">
        {residentScans.map((scan) => (
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-ink/[0.62]">{scan.detail}</p>
