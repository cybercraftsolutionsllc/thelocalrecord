...
export default async function LocalityReviewPage({ params }: LocalityReviewPageProps) {
  const { slug } = await params;
  const [data, entries] = await Promise.all([getLocalityData(slug), getReviewData(slug)]);
      ...
      <div className="rounded-[2rem] bg-white p-8 shadow-card">
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <h1 className="font-serif text-4xl text-moss">Unofficial-source review</h1>
            <p className="max-w-3xl text-base leading-8 text-ink/75">
              Official township and government sources publish automatically. This queue is reserved for
              unofficial-source items that still need human review.
            </p>
          </div>
